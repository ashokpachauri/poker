...
if ($action == 'createplayer')
{
	...
	if ($error == false)
	{
		...
		$result = $pdo->query("INSERT INTO " . DB_PLAYERS . " SET banned = '0', username = '".$usr."', approve = '".$approve."', email = '".$email."', GUID = '".$GUID."', lastlogin = '".$time."' , datecreated = '".$time."' , password = '".$pwd."', sessname = '".$sessname."', avatar = 'avatar.jpg', ipaddress = '".$ip."' ");
		...
		if (APPMOD == 1) // REQUIRE EMAIL VERIFICATION
		{
			...
			$url      = str_replace('index.php', "approve.php?&approval=$appcode", 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME']);
			$contents = CREATE_APPROVAL_EMAIL_CONTENT . ' ' . $url;

			OPS_mail($email, __( 'Player Activation Email', 'core' ), $contents);
