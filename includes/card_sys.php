...
foreach ($cardNumbers as $cNum)
{
	foreach (array('diamond', 'spade', 'heart', 'club') as $cType)
	{
		...
		$playerCard[$cNum . $cTypeShortUc . '-anim']          = $opsTheme->viewPart('poker-card-visible-animated');
		$playerCard[$cNum . $cTypeShortUc . '.png' . '-anim'] = $opsTheme->viewPart('poker-card-visible-animated');
		...
		$tableCard[$cNum . $cTypeShortUc . '-anim']          = $opsTheme->viewPart('poker-card-table-visible-animated');
		$tableCard[$cNum . $cTypeShortUc . '.png' . '-anim'] = $opsTheme->viewPart('poker-card-table-visible-animated');
	}
}
...
if ($game_style == GAME_TEXAS)
{
    ...
    $playerCard['facedown-anim']     = $opsTheme->viewPart('poker-card-hidden');
    $playerCard['facedown.png-anim'] = $opsTheme->viewPart('poker-card-hidden');
}
else
{
    ...
    $playerCard['facedown-anim']     = $opsTheme->viewPart('poker-card-hidden') . $opsTheme->viewPart('poker-card-hidden');
    $playerCard['facedown.png-anim'] = $opsTheme->viewPart('poker-card-hidden') . $opsTheme->viewPart('poker-card-hidden');
}
