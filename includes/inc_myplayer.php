...
if ($action == 'update_settings' && $plyrname != '')
{
    $sql = 'UPDATE ' . DB_PLAYERS . ' SET ';
    $setQuery = array();
    $valueArr = array();

    $skip = array('action', 'Submit');

    $addons->get_hooks(array(), array(
        'page'     => 'includes/inc_myplayer.php',
        'location'  => 'update_settings_start'
    ));
    ...
    $setQuery[] = "lastlogin = lastlogin";
    $sql       .= implode(',', $setQuery);
    $sql       .= " WHERE username = '{$plyrname}'";
...
$opsTheme->addVariable('stat', $statsr);
