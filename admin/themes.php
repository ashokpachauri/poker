...
if (isset($_POST['make_default_theme'], $_POST['theme'], $_POST['default']))
{
	$theme = str_replace('/[^A-Za-z0-9_-]/i', '', $_POST['theme']);
	$themeDir = "themes/{$theme}";
	...
	if (file_exists($themeDir))
	{
		$pdo->query("UPDATE " . DB_SETTINGS . " SET Xvalue = '{$theme}' WHERE setting = 'deftheme' AND Xkey = 'DEFTHEME'");
		echo '<script type="text/javascript">parent.document.location.href = "?admin=themes&defaulted=yes";</script>';
		die();
	}
}
...
if (class_exists('ZipArchive'))
	...
	$install = '<p>' . __( 'You need to enable Zip extension in your server', 'core' ) . '</p>';
...
foreach ($themeFolders as $themeFolder)
{
	...
	$empty = '<small>' . __( 'empty', 'core' ) . '</small>';
	...
	{
		if ($default)
		$infos->default = $opsTheme->viewPart('admin-theme-defaulted-button');
		else
			$infos->default = $opsTheme->viewPart('admin-theme-default-button');
	}
	else
		$infos->default = '';
	...
	if ($update)
		...
		$infos->update = '<span>' . __( 'Up to date', 'core' ) . '</span>';
